...
public class MobAttack1 : StateMachineBehaviour, IHitBoxResponder {
    ...
    public void CollisionWith(Collider collider)
    {
        ...
        if(null == hurtBox )
            hurtBox = collider.GetComponentInChildren<HurtBox>();
        if (null == hurtBox)
        ...
        if (null == hr)
            hr = collider.GetComponentInChildren<HitReaction>();
        if (null == hr)
            hr = collider.GetComponentInParent<HitReaction>();

        if (hr != null)
            hr.TakeDamage(damage, "Reaction", knuckBackDir);
    }

    // OnStateEnter is called when a transition starts and the state machine starts to evaluate this state
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        hitBox = animator.GetComponentInChildren<HitBox>();   //HitBox 는 스크립트 이름
